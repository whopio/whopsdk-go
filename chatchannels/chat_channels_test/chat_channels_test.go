...
func TestChatChannelsListWithWireMock(
	t *testing.T,
) {
	...
	_, invocationErr := client.ChatChannels.List(
		...
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestChatChannelsListWithWireMock"}},
		),
	)
...
func TestChatChannelsRetrieveWithWireMock(
	...
	_, invocationErr := client.ChatChannels.Retrieve(
		context.TODO(),
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestChatChannelsRetrieveWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestChatChannelsRetrieveWithWireMock", "GET", "/chat_channels/id", nil, 1)
}

func TestChatChannelsUpdateWithWireMock(
	t *testing.T,
) {
	WireMockBaseURL := os.Getenv("WIREMOCK_URL")
	if WireMockBaseURL == "" {
	...
	_, invocationErr := client.ChatChannels.Update(
		context.TODO(),
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestChatChannelsUpdateWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestChatChannelsUpdateWithWireMock", "PATCH", "/chat_channels/id", nil, 1)
}
