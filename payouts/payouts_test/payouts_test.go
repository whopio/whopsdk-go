...
func TestPayoutsListWithWireMock(
	...
	VerifyRequestCount(t, "TestPayoutsListWithWireMock", "GET", "/payouts", nil, 1)
}

func TestPayoutsCreateWithWireMock(
	t *testing.T,
) {
	WireMockBaseURL := os.Getenv("WIREMOCK_URL")
	if WireMockBaseURL == "" {
	...
	_, invocationErr := client.Payouts.Create(
		...
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestPayoutsCreateWithWireMock"}},
		),
	)
...
func TestPayoutsRetrieveWithWireMock(
	...
	_, invocationErr := client.Payouts.Retrieve(
		context.TODO(),
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestPayoutsRetrieveWithWireMock"}},
		),
	)
