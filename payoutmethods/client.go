// Code generated by Fern. DO NOT EDIT.

package payoutmethods

import (
	context "context"
	http "net/http"

	whopsdk "github.com/whopio/whopsdk-go"
	core "github.com/whopio/whopsdk-go/core"
	internal "github.com/whopio/whopsdk-go/internal"
	option "github.com/whopio/whopsdk-go/option"
)

type Client struct {
	WithRawResponse *RawClient

	options *core.RequestOptions
	baseURL string
	caller  *internal.Caller
}

func NewClient(options *core.RequestOptions) *Client {
	if options.APIVersionDate == nil {
		apiVersionDateDefault := "2026-09-11-1"
		options.APIVersionDate = &apiVersionDateDefault
	}
	return &Client{
		WithRawResponse: NewRawClient(options),
		options:         options,
		baseURL:         options.BaseURL,
		caller: internal.NewCaller(
			&internal.CallerParams{
				Client:         options.HTTPClient,
				MaxAttempts:    options.MaxAttempts,
				DisableRetries: options.DisableRetries,
			},
		),
	}
}

// Returns a list of active payout methods configured for a company, ordered by most recently created.
//
// Required permissions:
//   - `payout:destination:read`
//
// Example:
//
//	request := &whopsdk.ListPayoutMethodRequest{
//	    First: whopsdk.Int(
//	        42,
//	    ),
//	    Last: whopsdk.Int(
//	        42,
//	    ),
//	    AccountID: "biz_xxxxxxxxxxxxxx",
//	}
//	client.PayoutMethods.ListPayoutMethod(
//	    context.TODO(),
//	    request,
//	)
func (c *Client) ListPayoutMethod(
	ctx context.Context,
	request *whopsdk.ListPayoutMethodRequest,
	opts ...option.RequestOption,
) (*core.Page[*string, *whopsdk.PayoutMethodListItem, *whopsdk.ListPayoutMethodResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		c.baseURL,
		"https://api.whop.com/api/v1",
	)
	endpointURL := baseURL + "/payout_methods"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	headers := internal.MergeHeaders(
		c.options.ToHeader(),
		options.ToHeader(),
	)
	prepareCall := func(pageRequest *core.PageRequest[*string]) *internal.CallParams {
		if pageRequest.Cursor != nil {
			queryParams.Set("after", *pageRequest.Cursor)
		}
		nextURL := endpointURL
		if len(queryParams) > 0 {
			nextURL += "?" + queryParams.Encode()
		}
		return &internal.CallParams{
			URL:             nextURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        pageRequest.Response,
			ErrorDecoder:    internal.NewErrorDecoder(whopsdk.ErrorCodes),
		}
	}
	readPageResponse := func(response *whopsdk.ListPayoutMethodResponse) *core.PageResponse[*string, *whopsdk.PayoutMethodListItem, *whopsdk.ListPayoutMethodResponse] {
		var zeroValue *string
		var next *string
		if response.PageInfo != nil {
			next = response.PageInfo.EndCursor
		}
		results := response.GetData()
		return &core.PageResponse[*string, *whopsdk.PayoutMethodListItem, *whopsdk.ListPayoutMethodResponse]{
			Results:  results,
			Response: response,
			Next:     next,
			Done:     next == zeroValue || *next == "",
		}
	}
	pager := internal.NewCursorPager(
		c.caller,
		prepareCall,
		readPageResponse,
	)
	return pager.GetPage(ctx, request.After)
}

// Retrieves the details of an existing payout method.
//
// Required permissions:
//   - `payout:destination:read`
//
// Example:
//
//	request := &whopsdk.RetrievePayoutMethodRequest{
//	    ID: "potk_xxxxxxxxxxxxx",
//	}
//	client.PayoutMethods.RetrievePayoutMethod(
//	    context.TODO(),
//	    request,
//	)
func (c *Client) RetrievePayoutMethod(
	ctx context.Context,
	request *whopsdk.RetrievePayoutMethodRequest,
	opts ...option.RequestOption,
) (*whopsdk.PayoutMethod, error) {
	response, err := c.WithRawResponse.RetrievePayoutMethod(
		ctx,
		request,
		opts...,
	)
	if err != nil {
		return nil, err
	}
	return response.Body, nil
}
