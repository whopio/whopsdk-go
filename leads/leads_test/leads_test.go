...
func TestLeadsListWithWireMock(
	t *testing.T,
) {
	...
	_, invocationErr := client.Leads.List(
		...
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestLeadsListWithWireMock"}},
		),
	)
...
func TestLeadsCreateWithWireMock(
	...
	_, invocationErr := client.Leads.Create(
		...
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestLeadsCreateWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestLeadsCreateWithWireMock", "POST", "/leads", nil, 1)
}

func TestLeadsRetrieveWithWireMock(
	t *testing.T,
) {
	WireMockBaseURL := os.Getenv("WIREMOCK_URL")
	if WireMockBaseURL == "" {
	...
	_, invocationErr := client.Leads.Retrieve(
		context.TODO(),
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestLeadsRetrieveWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestLeadsRetrieveWithWireMock", "GET", "/leads/lead_xxxxxxxxxxxxx", nil, 1)
}

func TestLeadsUpdateWithWireMock(
	t *testing.T,
) {
	WireMockBaseURL := os.Getenv("WIREMOCK_URL")
	if WireMockBaseURL == "" {
	...
	_, invocationErr := client.Leads.Update(
		context.TODO(),
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestLeadsUpdateWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestLeadsUpdateWithWireMock", "PATCH", "/leads/lead_xxxxxxxxxxxxx", nil, 1)
}
